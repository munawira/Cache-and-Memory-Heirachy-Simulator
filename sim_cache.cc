...
int readwrite( unsigned long full_address, char* L1_tag, char* L2_tag,unsigned long L1_index,unsigned long L2_index);/* Implements writing into L1, L2 and stream buffers*/
int evictL1(unsigned long L1_index,int L1_blockid,unsigned long L2_index, char* L2_tag,  unsigned long address );
int L1readmiss(char* L1_tag,unsigned long L1_index, unsigned long L2_index,  char* L2_tag, unsigned long address );
int L2readmiss(unsigned long L2_index, char* tag, unsigned long address);
...
int main(int argc, char* argv[])
{
	...
	cout<<"		trace_file:		"<<argv[10]<<"\n";

	
	trace = fopen(argv[10], "r");
	...
	while(fscanf(trace, "%c %lx", &command,&full_address)>0)
	{
	...
	{	written =1;
		...
		readwrite(full_address,L1_tag,L2_tag,L1_index,L2_index );
	...
	for(int j =0; j<L1_ASSOC; j++)
	{
		if(L1.LRU[i][j] == k)
		{
		strcpy(printtag,L1.set[i][j].getTag());
		
		if(L1.set[i][j].getDirty())
		dirty = 'D';
		else
		dirty = ' ';

		if(L1.set[i][j].getValid())
		cout <<" \t "<< printtag << " " << dirty<<' ' ;
		else
		cout <<" \t "<< "    -    " << " " << dirty<<' ' ;
		}
	...
	for(int k =1; k<=sblru; k++)
	{
	for(i =0; i< L1_PREF_N; i++)
	{
	if(SB_L1[i].getLRU() == k)
	{
	
	for(int j =0; j<L1_PREF_M; j++)
	
	{
		
		sprintf(printaddr,"%lx",SB_L1[i].SB[j].getAddress());
		if(SB_L1[i].SB[j].getValid())
		cout <<"	"<< printaddr;
		else
		cout <<"	"<< "    -    ";
	...
	for(int j =0; j<L2_ASSOC; j++)
	{
		if(L2.LRU[i][j] == k)
		{
		strcpy(printtag,L2.set[i][j].getTag());
		if(L2.set[i][j].getDirty())
		dirty = 'D';
		else
		dirty = ' ';
		if(L2.set[i][j].getValid())
		cout <<" \t "<< printtag << " " << dirty<<' ' ;
		else
		cout <<" \t "<< "    -    " << " " << dirty<<' ' ;
		}
	...
	for(int k =1; k<=sblru; k++)
	{	
	for(i =0; i< L2_PREF_N; i++)
	{
	
	if(SB_L2[i].getLRU() == k)
	{
	for(int j =0; j<L2_PREF_M; j++)
	
	{
		sprintf(printaddr,"%lx",SB_L2[i].SB[j].getAddress());
		if(SB_L2[i].SB[j].getValid())
		cout <<"	"<< printaddr;
		else
		cout <<"	"<< "    -    ";
	...
	cout<<"b. number of L1 read misses:          "<< L1readMiss<<"\n";
	cout<<"c. number of L1 writes:               "<<L1writes<<"\n";
	cout<<"d. number of L1 write misses:          "<<L1writeMiss<<"\n";
	printf("e. L1 miss rate:                      %f\n",L1missrate);
	cout<<"f. number of L1 writebacks:            "<<L1writeback<<"\n";

	cout<< "g. number of L1 prefetches:            "<<L1prefetch<<"\n";
	
	cout<< "h. number of L2 reads that did not originate from L1 prefetches:	"<<L2reads<<"\n";
	cout<<"i. number of L2 read misses that did not originate from L1 prefetches:	"<<L2readMiss<<"\n";
	cout<< "j. number of L2 reads that originated from L1 prefetches:		"<<L2reads_prefetch<<"\n";
	cout<<"k. number of L2 read misses that originated from L1 prefetches:	"<<L2readmiss_prefetch<<"\n";
	cout<<"l. number of L2 writes:		            "<<L2writes<<"\n";
	cout<<"m. number of L2 write misses:          "<<L2writeMiss<<"\n";
	printf("n. L2 miss rate:                      %f\n",L2missrate);
	cout<<"o. number of L2 writebacks:            "<<L2writeback<<"\n";
	cout<<"p. number of L2 prefetches:	          "<<L2prefetches<<"\n";
	cout<<"q. total memory traffic:             "<< memtraffic<<"\n";
...
int readwrite( unsigned long full_address, char* L1_tag, char* L2_tag,unsigned long L1_index,unsigned long L2_index)
{
	//int miss =0;
	char index[20];
	int streamId = 0;
	...
	if(blockid == -1)
	{
		...
		L1readmiss(L1_tag, L1_index, L2_index, L2_tag,full_address);
...
{	
	...
	if(emptyBlock == -1) 
	{
		...
		if(victimDirty == 1)
		evictL1(L1_index, blockid, L2_index,L2_tag, address); /* Evicts the least recently used block in the set and also does a write back if the block is dirty*/
		...
		success = L2.cacheSearch(L2_tag, L2_index);
			...
			{
			L2readMiss++;
			L2readmiss(L2_index,L2_tag,address);
			}
			else
			{
#ifdef debuf
...
int evictL1(unsigned long L1_index,int L1_blockid,unsigned long L2_index, char* L2_tag,  unsigned long address )
{
	...
	if(L2_presence == 1)
	{
		...
		{
		L2readmiss(victim_index,victimTag,victimAddress);
...
int L1preFetch(int bufferId, char* tag,unsigned long L2_index,unsigned long full_address, unsigned long intAddress)
{
	...
	prefetch =1;
	L2readmiss(index,L2_tag,full_address);
	prefetch =0;
